...
;(function($){
	...
	$.weditor = {
        ...
        button : {
        	...
        	forecolor             : {hasLayer : true,  ifrmLeft : "0", "mouseUp" : "1", action : "openLayer",   title : "글자색상"},
        	forecolormore         : {hasLayer : true,  ifrmLeft : "0", "mouseUp" : "2", action : "openLayer",   title : "더보기"},
        	backcolor             : {hasLayer : true,  ifrmLeft : "0", "mouseUp" : "1", action : "openLayer",   title : "배경색상"},
        	backcolormore         : {hasLayer : true,  ifrmLeft : "0", "mouseUp" : "2", action : "openLayer",   title : "더보기"},
        	...
        	blockquote            : {hasLayer : true,  ifrmLeft : "0", "mouseUp" : "0", action : "openLayer",   title : "필드셋"},
        	...
        	file                  : {hasLayer : true,  ifrmLeft : "0", "mouseUp" : "2", action : "uploadImage", title : "파일"}
        },
	...
	$.weditor.create = function($target, options){
		...
		this.el.$editorDoc.write(mappingValue(this.template.docHtml, [this.options.fontFamily, this.options.fontSize, this.data.initTag]));
		this.el.$editorDoc.close();
		this.el.$editorDoc.designMode = "on";
		...
		$.each(this.toolbar[this.options.toolbar], function(index, unit){
			if(index!=0) ui.push($.weditor.template.separator);
			$.each(unit, function(index2, buttonName){
				if(buttonName=="fontname"){
					ui.push($.weditor.event.makeButton(buttonName, "<span>" + $.weditor.util.getFirsArrayData($.weditor.options.fontFamily) + "</span>"));
                }else if(buttonName=="fontsize"){
                	ui.push($.weditor.event.makeButton(buttonName, $.weditor.options.fontSize));
                }else{
                	ui.push($.weditor.event.makeButton(buttonName, ""));
                }
				
				if(buttonName=="forecolor" || buttonName=="backcolor"){
					ui.push($.weditor.event.makeButton(buttonName+"more", ""));
				}
			});
		});
		
		if(ui.length>0) this.el.$header.find("ul").append(ui.join(""));
		...
		.on("mouseover", function(event) {
			$.weditor.event.changeButton($(this), "over");
		})
		.on("mouseout", function(event) {
			$.weditor.event.changeButton($(this), "out");
		})
		.on("mousedown", function(event) {
			$.weditor.event.changeButton($(this), "down");
		})
		.on("mouseup", function(event) {
		    if($.weditor.button[$(this).find("button").attr("class")].mouseUp=="1"){
		    	$.weditor.event.changeButton($(this), "up");
		    }
		})
        ...
        // conversion mode
        $.each(this.options.conversionMode, function(){
        	$("ul li." + this + "Mode", $.weditor.el.$footer).removeClass("none");
        });
        
    	$("ul li", this.el.$footer).click(function() {
    		var skip = false;
    		if($(this).hasClass("editorMode") && $(this).hasClass("off")){
    			$.weditor.el.$textEditor.hide();
    			$.weditor.el.$editor.show();
    			$.weditor.el.$headerbg.hide();
                if($("ul li.htmlMode", $.weditor.el.$footer).hasClass("on")){
                	$.weditor.setContent($.weditor.getContent("text"));
                }else if($("ul li.textMode", $.weditor.el.$footer).hasClass("on")){
                	$.weditor.setContent($.weditor.el.$textEditor.val(), "text");
                }
                $.weditor.mode = "editor";
            }else if($(this).hasClass("htmlMode") && $(this).hasClass("off")){
            	$.weditor.el.$textEditor.hide();
            	$.weditor.el.$editor.show();
            	$.weditor.el.$headerbg.show();
                if($("ul li.editorMode", $.weditor.el.$footer).hasClass("on")){
                	$.weditor.setContent($.weditor.getContent(), "text");
                }else if($("ul li.textMode", $.weditor.el.$footer).hasClass("on")){
                	$.weditor.setContent($.weditor.el.$textEditor.val(), "text");
                }
                $.weditor.mode = "html";
            }else if($(this).hasClass("textMode") && $(this).hasClass("off")){
    	...
    	$("ul li." + this.options.defaultConversionMode + "Mode", $.weditor.el.$footer).trigger("click");
    	
        if(this.options.resize){
        	this.el.$footer.find("div")
        	.addClass("resize")
            .bind("mousedown", function(event) {
            	$.weditor.resize.start(event)
    		}).html("<span class=\"resizemore off\"/>");
        }
	};
	
	$.weditor.event = {
		makeButton : function(buttonName, addStr){
			return "<li title=\"" + $.weditor.button[buttonName].title + "\"><button class=\"" + buttonName + "\">" + addStr + "</button></li>";
		},
		changeButton : function($t, mode, p) {
			if(typeof($t) =="string") $t = $($t, this.el.$button);
			var className = $t.attr("class");
			if(mode=="down" && className=="down" && typeof(p)=="undefined"){
			    if(className="up") $t.attr("class", "up");
			} else if((mode=="up" && className=="down") || className!="down") {
			    if(className!=mode) $t.attr("class",  mode);
	        }
		},
		clickButton : function(buttonName){
			$.weditor.action[$.weditor.button[buttonName].action].apply(null, [buttonName]);
		}
	};
		...
		makeLayer : function(buttonName){
			...
			switch (buttonName){
				...
				case "backcolormore" :
					...
					break;
				case "blockquote" :
					$.each($.weditor.data.blockquote, function(){
				...
				case "link" :
					...
					break;
				case "character" :
					$.each($.weditor.data.character.tab, function(index){
			            var $li = $("<li><button><span>" + this + "</span></button><div><ul></ul></div></li>");
			            var char = [];
			            $.each(unescape($.weditor.data.character.value[index]).split(' '), function(){
			            	char.push("<li class=\"has\"><button><span>" + this + "</span></button></li>");
			            });
			            
			            var _n = $.weditor.data.character.max-char.length;
			            for(var i=0;i<_n;i++){
	...
	$.weditor.resize = {
		...
		move : function(e){
			...
			$.weditor.el.$footer.find(".resizemore").removeClass("off").addClass("on");
		},
		stop : function (){
			$(document).off("mousemove", $.weditor.resize.move).off("mouseup", $.weditor.resize.stop);
			$.weditor.el.$footer.find(".resizemore").removeClass("on").addClass("off");
