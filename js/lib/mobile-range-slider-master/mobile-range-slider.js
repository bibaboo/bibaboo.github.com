...
*/
var zoomSlider;
function makeZoomSlider($target, option){
	var _option = $.extend({value : 0,min	: 1,max : 100}, option||{});
	if($("#zoomSlider").length>0){
		zoomSlider.setValue(_option.value);
	}else{
		$target.before($.tmpl(zoomSlider));
		zoomSlider = new MobileRangeSlider('zoomSlider', {
			value: _option.value,
			min: _option.min,
			max: _option.max,
			change: function(percentage) {
				if(_option.callback){
	    		  _option.callback(percentage); 
				}else{
					var _w = 100 + percentage;
					$target.width(_w+"%");
				}
			}
	    });
