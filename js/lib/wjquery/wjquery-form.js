	...
	var WFORM = new(function() {
    	...
    	function get(){
    		...
    		if($t.is("input:radio")){
    			...
    			return $("input:checkbox[name='" + $t.attr("name") + "']:checked").map(function(){return this.value;}).get();
    		}else{
    			return $(this).val();
    		}
    	}
    	...
    	function make(type, options){
    		...
    		if(type == "select"){
    			$t.append($("<select />").attr({id:_options.id, name:_options.name, class:_options.class}));
